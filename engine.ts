...
const swidth = 512
const sheight = 512
const xScale = swidth / width
const yScale = sheight / height
...
function prt(str: string, x: number, y: number, pen: number, paper: number = undefined, wrap = true) {
...
window.onload = () => {
    canvas.addEventListener('mousemove', (evt) => {
        ...
        mouse.x = Math.floor((evt.clientX - rect.left) / xScale)
        mouse.y = Math.floor((evt.clientY - rect.top) / yScale)
    }, false)
